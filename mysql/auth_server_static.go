...
func InitAuthServerStatic() {
    ...
    if *mysqlAuthServerStaticFile != "" && *mysqlAuthServerStaticString != "" {
        ...
        log.Errorf("Both mysql_auth_server_static_file and mysql_auth_server_static_string specified, can only use one.")
        log.Exit(-1)
    }
...
func RegisterAuthServerStaticFromParams(file, str string) {
    ...
    if len(authServerStatic.Entries) <= 0 {
        log.Errorf("Failed to populate entries from file: %v", file)
        log.Exit(-1)
    }
